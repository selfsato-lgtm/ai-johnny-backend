...
// 会員リスト（lovequest-mastersと共通のGoogle Sheet）でステータスを確認し、
// 有効なら署名付きセッションCookieを発行する。

const { createSessionToken, SESSION_COOKIE_NAME, SESSION_MAX_AGE_MS } = require('../../lib/session');
...
module.exports = async function handler(req, res) {
  ...
  if (error) {
    res.writeHead(302, { Location: 'https://lovequest-masters.com/?login=cancelled' });
    res.end();
    return;
  }
  if (!code) {
  ...
  try {
    ...
    if (!email || !userInfo.email_verified) {
      res.writeHead(302, { Location: 'https://lovequest-masters.com/?login=failed' });
      res.end();
      return;
    }
    ...
    if (!active) {
      res.writeHead(302, { Location: 'https://lovequest-masters.com/?login=inactive' });
      res.end();
      return;
    }
